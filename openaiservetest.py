...
async def generate_conversations_data(book_name:str,role_name:str,cleaned_chunks_data_path:str,formatted_data_save_path:str):#调用deepseek生成问题和回答，进行构造数据集
    ...
    role_description_response = client.chat.completions.create(
        model = "deepseek-chat",
    ...
    with open(formatted_data_save_path,'w',encoding='utf-8') as f:
        json.dump(formatted_data, f, ensure_ascii=False, indent=2)

asyncio.run(generate_conversations_data('见春天','魏清越','output_text_data/cleaned_chunks_data.json','output_text_data/formatted_data.json'))
